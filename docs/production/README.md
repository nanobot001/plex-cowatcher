...
Roll out in this order: back up live SQLite; deploy disabled; inspect `audiobook-proof --action status`; run a dry-run canary for one audiobook; run the same canary with `--apply --confirm`; verify its matching chapter revision, Progress output, audit redaction, and health summary; then enable the worker and restart PM2. For existing `unsupported_multi_file` jobs, use `audiobook-proof --action reevaluate --audiobook-id <id>` first, then add `--apply --confirm` only after an explicit one-process multi-file override reports `READY_FOR_FILE_BOUNDARY_CHAPTER_PROOF` or the reviewed general-analyzer reason `READY_FOR_MULTI_FILE_PROOF`. Exact current fallback jobs and exact legacy `MULTI_FILE_INVALID_CHAPTERS` jobs use the file-boundary reason; every mismatch remains unresolved under its specific `MULTI_FILE_*` reason. Keep the recurring deployment flag false during targeted canaries. The worker processes at most one file checkpoint per cycle with concurrency one. After restart, run `npm run verify:live-dashboard`.
