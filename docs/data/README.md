...
- `audiobook_proof_jobs` contains exactly one durable job per `(audiobook_id, media_revision)`. It stores only queue state, lease/retry timing, allowlisted outcomes, and bounded diagnostics; private paths remain in revision-manifest items.
- `audiobook_proof_file_jobs` contains one restart-safe checkpoint per ordered manifest item for multi-file proof. It may expose only counts, states, timing, and allowlisted reason codes; private paths and analyzer output remain confined to SQLite.
- Multi-file proof is separately gated by `AUDIOBOOK_PROOF_MULTI_FILE_ENABLED`, which defaults false. While disabled, `unsupported_multi_file` remains an explicit fallback classification and no per-file proof work is materialized.
- `audiobook_proof_state` persists the single-worker lease and completion-based 15-minute schedule. Expired job leases recover into retry or terminal state without creating replacement rows.
