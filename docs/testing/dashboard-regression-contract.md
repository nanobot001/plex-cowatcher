...
- Audiobook Overview digests, Progress cards, and shared detail must consume the same progress-quality resolver. Missing `view_offset` plus corroborated grouped listening-time high-water evidence may produce an explicitly approximate position; lower reset rows cannot erase that position, inflate listening time, or let a stale source percentage produce verified chapter completion.
