...
- Every visible Audiobook progress surface consumes only the additive canonical per-listener projection. Current position is the resume value, a different furthest position is secondary and never replaces current, a trusted rewind is labeled `Revisiting`, approximate/stale evidence is qualified, and unavailable position renders as unknown without a fabricated meter.
- Audiobook Overview digests and Timeline activity render each observation/session's historical `session_as_of` projection rather than today's latest book position. Gantt width remains explicit session time, and audiobook session grouping never crosses book identity.
- Audiobook Progress and shared detail visibly agree per listener on current/furthest positions, chapter states, quality, and reason. Chapter lanes use canonical `passed`, `in_progress`, `revisiting`, `probably_passed`, `explicitly_completed`, and `unknown` states rather than inferring completion from touched playback rows.
- Media Explorer and Continue Consuming render current position as the primary accessible meter and expose a different canonical furthest position only as secondary context. Raw `percentComplete`, `viewOffset`, and `duration` never determine visible Audiobook progress in browser code.
- People and Overview keep completed playback observations, passed Audiobook chapters, and completed Audiobook books as separate visible facts; household progress is never averaged or merged across listeners.
- Missing `view_offset` plus corroborated grouped listening-time high-water evidence may produce an explicitly approximate position; lower reset rows cannot erase that position, inflate listening time, or let a stale source percentage produce verified chapter completion.
