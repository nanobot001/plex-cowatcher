...
| `project.plex_historical_backfill` | `node dist/cli/cli.js plex-historical-backfill` | *(none)* | `write_action` | Dry-run by default; preserves aggregate last-view compatibility and adds paginated Plex play-history recovery through `--history-source play-history`. Use `--media-type movie|episode|all`; CLI-only; no Plex mutation. |
...
- `project.plex_historical_backfill` is intentionally CLI-only. `--history-source aggregate` remains the default compatibility behavior. `--history-source play-history` pages server-local account history, stores each stable history row as an additive `plex_api_history` archive event, supports `--date-from`, `--date-to`, `--page-size`, `--run-id`, and `--report`, and interval-links exact Tautulli overlaps without copying or overwriting them. Dry-run writes no recovery rows; apply requires `--apply --confirm` and a verified SQLite backup. Output includes structured Tautulli interval readiness (`ready`, `partial`, `missing`, or `not_applicable`) so legacy rows can be enriched by the existing confirmed Tautulli backfill before Plex apply. Public output never includes Plex account IDs, history keys, tokens, upstream URLs, or private paths.
