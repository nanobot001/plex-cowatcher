...
| `project.plex_historical_backfill` | `node dist/cli/cli.js plex-historical-backfill` | *(none)* | `write_action` | Dry-run by default; reads per-user Plex movie or episode visibility and last-view metadata, then applies at most one exact-GUID historical observation per media item before the cutoff. Use `--media-type movie|episode|all`; CLI-only; no Plex mutation. |
...
- `project.plex_historical_backfill` is intentionally CLI-only. Dry-run does not write SQLite; apply requires both `--apply` and `--confirm`, preserves raw snapshots and derived observations separately, supports exact-keyed movie/episode recovery through `--media-type`, and labels derived observations with `plex_historical_last_view`.
