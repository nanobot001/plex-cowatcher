...
import type { DashboardActivityItem, DashboardCategory } from "../types/api.js";
...
const filterSchema = z.object({
  ...
  category: z.enum(["movie", "tv", "classic_tv", "anime", "audiobook", "other"]).optional(),
  library: z.string().optional(),
  completed: z.preprocess((value) => value === "true" ? true : value === "false" ? false : value, z.boolean().optional()),
  search: z.string().max(200).optional(),
  limit: z.preprocess((value) => value === undefined ? 50 : Number(value), z.number().int().min(1).max(20000)),
...
function isRecognizedExplorerItem(item: DashboardActivityItem): boolean {
  return item.category !== "other" && Boolean(explorerTitle(item).trim()) && (item.category === "audiobook" || Boolean(item.libraryName));
}
...
export function deriveDashboardCategory(mediaType: string, libraryName?: string): { category: DashboardCategory; label: string; derived: boolean } {
...
export class DashboardService {
  ...
  getActivity(input: unknown): { items: DashboardActivityItem[]; total: number; limit: number; offset: number } {
    ...
    return { items: rows.map((row) => this.mapActivity(row)), total, limit: p.limit, offset: p.offset };
  }

  getContinueWatching(input: unknown) {
    const p = parseFilters(input);
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
    const all = this.getActivity({ ...(input as object), limit: 1000, offset: 0 }).items;
  ...
  getMedia(input: unknown) {
    const p = parseFilters(input);
    const all: DashboardActivityItem[] = [];
    let offset = 0;
    do {
      const page = this.getActivity({ ...(input as object), limit: 200, offset }).items;
      all.push(...page);
      offset += page.length;
    ...
    for (const item of all) {
      ...
      const groupRatingKey = item.category === "audiobook"
        ...
        : (item.category === "tv" || item.category === "classic_tv" || item.category === "anime")
          ...
          : item.ratingKey;
      const group = groups.get(key) ?? { ...item, title, showTitle: undefined, displayTitle: title, groupRatingKey, plays: 0, distinctItems: new Set<string>(), people: new Set<number>(), latestWatchedAt: item.watchedAt, artworkUrl: "/api/artwork/" + encodeURIComponent(groupRatingKey) };
      group.plays += 1;
  ...
  getDetail(ratingKey: string) {
    const plays = this.getActivity({ ratingKey, limit: 200, offset: 0 }).items;
  ...
  private mapActivity(row: any): DashboardActivityItem {
    ...
    const artworkKey = (category.category === "tv" || category.category === "anime" || category.category === "classic_tv")
      ? (row.grandparent_rating_key ?? row.rating_key)
      : (category.category === "audiobook" ? (row.parent_rating_key ?? row.grandparent_rating_key ?? row.rating_key) : row.rating_key);
